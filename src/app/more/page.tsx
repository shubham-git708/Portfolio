          ...
          <div className="relative z-10">
            ...
            {portfolioConfig.experience.map((exp, idx) => (
              <FramerWrapper key={idx} y={0} x={0} className="mb-4">
                <div className="font-bold text-base text-primary">{exp.title} <span className="font-normal text-sm text-muted-foreground">@ {exp.company} ({exp.period}, {exp.location})</span></div>
            ...
            {portfolioConfig.internships.map((intern, idx) => (
              <FramerWrapper key={idx} y={0} x={0} className="mb-4">
                <div className="font-bold text-base text-primary">{intern.title} <span className="font-normal text-sm text-muted-foreground">@ {intern.company} ({intern.period}, {intern.location})</span></div>
                <ul className="list-disc ml-6 mt-1 text-base text-gray-700">
